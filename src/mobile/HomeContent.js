...
function mobileMasthead(props) {
  ...
  return (
    <div>
      ...
      <div className={ classes.titleBarMobile }>
        { 'ricktoews.me' }
{/* Filter toggle icon */}
...
const MobileMasthead = withRouter(withStyles(styles)(mobileMasthead));


function HomeContent({ classes, content }) {
  const [ state, dispatch ] = useContext(Context);
  return (
  <div>
    <MobileMasthead id={'home'}/>

    <div className="home-container">
      { content.filter(item => state.filter === '' || item.category === state.filter).map((post, key) => <HomePost key={key} post={post}/>)}
    </div>
  </div>
  );
}
